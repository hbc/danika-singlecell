...
Danika's single cell run. There are two sets of HSC cells, young and old, along with some pooled samples of 10 and 1,000 cells for each condition to compare to. There is a positive control, but I'm
not sure what it is.

The reads are paired end, 75bp reads. The library is not great for this though,
since the average insert size is only 50 bp.
...
sorting and replotting shows this more clearly.
...
ggplot(z, aes(x=factor(Name), y=Mapped)) +
    ...
    ylab("mapped reads") + xlab("")
```
These are reads mapped, not
reads mapping to genes. If we look at the total reads mapping to genes in
each sample, the counts are even lower:
...
We can see as similar distribution in the mapping rate:
...
The gene detection saturation should look more like a asymptotic curve, but it
doesn't. There is a small cluster of cells near the origin that look like
failures, which is expected, but there are many cells with a small number of
reads mapped in the samples.
...
## Exonic mapping rate
The exonic mapping rate is lower than we would normally expect as well.
...
```{r spike-in-identity}
ggplot(ercc, aes(sample, mapped + unmapped)) + geom_point() + facet_wrap(~ spike) +
    scale_y_sqrt() +
    theme_bw(base_size=10) +
    xlab("sample") +
    ylab("reads mapped") +
...
ssummarydata = ssummarydata[keep,]
...
There could possibly be some subgroups here.

## subgroups

I had sent along this image:
[](../images/heatmap-counts-markers.png)

Which is a heatmap of the log normalized counts of marker genes in a small set
of the cells.
...
I don't agree there are four major cell types here,
...
## simple HSC classifier
Maybe there are populations of cells within the groups that are a big blob
on the PCA.

From Danika:

> I would most likely use this hierarchy for narrowing cells down to HSCs:
>
> Ptprc,+
> Kit,+
> Atxn1,+
...
So we created a very simple classifier that gives a positive score for the genes
we expect to be expressed and a negative score for any genes that we don't
expect to see expressed.

```{r simple-hsc-classifier}
scounts = nbt@data
important = c("Slamf1")
positive_known = c("Ptprc", "Kit", "Atxn1")
negative_known = c("Cd34", "Flt3")
scores = colSums(scounts[c(positive_known, important),])
possible_hscs = scores > 5
for_heatmap <- possible_hscs[rownames(possible_hscs) %in% rownames(anno_df),]
for_heatmap_anno <- subset(anno_df, rownames(anno_df) %in% rownames(for_heatmap))
pheatmap(log(for_heatmap[rownames(for_heatmap_anno),] + 1),
         annotation_row=for_heatmap_anno, show_colnames=FALSE,
         cluster_rows=FALSE, fontsize=8, cluster_columns=FALSE)
```

## Now we will do differential expression between the young/old groups in this
## group of possible HSC cells.

## ```{r scde-possible-hscs}
## library(scde)
## n.cores=2
## possible_hsc_counts = counts[, colnames(possible_hscs)]
## possible_hsc_counts = possible_hsc_counts[rowSums(possible_hsc_counts > 0) > 0.1*ncol(possible_hsc_counts),]
## groups = factor(as.factor(summarydata[colnames(possible_hsc_counts),]$sampletype))
## names(groups) = rownames(summarydata[colnames(possible_hsc_counts),])
## o.ifm <- scde.error.models(counts=possible_hsc_counts,groups=groups,n.cores=n.cores,threshold.segmentation=T,save.crossfit.plots=F,save.model.plots=F,verbose=1);
## valid.cells <- o.ifm$corr.a >0;
## o.ifm <- o.ifm[valid.cells,];
## o.prior <- scde.expression.prior(models=o.ifm,counts=possible_hsc_counts,length.out=400,show.plot=F)
## ediff <- scde.expression.difference(o.ifm,possible_hsc_counts,o.prior,groups=groups,n.randomizations=100,n.cores=n.cores,verbose=1)
## ediff$pvalue = pnorm(-(abs(ediff$Z))) * 2
## ediff$padj = p.adjust(ediff$pvalue)
## ediff = ediff[order(abs(ediff$Z),decreasing=T),]
## ediff$symbol = conversion[match(rownames(ediff), conversion$ensembl_gene_id),"mgi_symbol"]
## write.table(ediff, file="possible_hsc_scde_de.txt", col.names=TRUE, quote=FALSE, row.names=TRUE)
## ```

## ```{r scde-definite-hscs}
## library(scde)
## n.cores=2
## definite_hscs = possible_hscs[,possible_hscs["Slamf1",] > 0 & possible_hscs["Cd48",] == 0]
## definite_hsc_counts = counts[, colnames(definite_hscs)]
## definite_hsc_counts = definite_hsc_counts[rowSums(definite_hsc_counts > 0) > 0.1*ncol(definite_hsc_counts),]
## groups = factor(as.factor(summarydata[colnames(definite_hsc_counts),]$sampletype))
## names(groups) = rownames(summarydata[colnames(definite_hsc_counts),])
## o.ifm <- scde.error.models(counts=definite_hsc_counts,groups=groups,n.cores=n.cores,threshold.segmentation=T,save.crossfit.plots=F,save.model.plots=F,verbose=1);
## valid.cells <- o.ifm$corr.a >0;
## o.ifm <- o.ifm[valid.cells,];
## o.prior <- scde.expression.prior(models=o.ifm,counts=definite_hsc_counts,length.out=400,show.plot=F)
## ediff <- scde.expression.difference(o.ifm,definite_hsc_counts,o.prior,groups=groups,n.randomizations=100,n.cores=n.cores,verbose=1)
## ediff$pvalue = pnorm(-(abs(ediff$Z))) * 2
## ediff$padj = p.adjust(ediff$pvalue)
## ediff = ediff[order(abs(ediff$Z),decreasing=T),]
## ediff$symbol = conversion[match(rownames(ediff), conversion$ensembl_gene_id),"mgi_symbol"]
## write.table(ediff, file="definite_hsc_scde_de.txt", col.names=TRUE, quote=FALSE, row.names=TRUE)
## ```

## Need some way to try to classify bad cells, cells that
## didn't work and other outliers. Also need some way to clean bad genes as well.

## total_reads = colSums(counts)
## detected = colSums(counts > 0)
## qplot(total_reads, detected)

## qplot(detected / total_reads) + scale_x_log10()


## qplot(summarydata$Mapped, summarydata$Mapping.Rate, size=summarydata$Genes.Detected)

## dim(summarydata)
## dim(subset(summarydata, Genes.Detected > 1000))
